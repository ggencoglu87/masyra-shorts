...
class ViralEngineV3Tests(unittest.TestCase):
    ...
    def test_v7_story_universe_prompts_and_memory_are_episode_ready(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            voiceover = (video_dir / "voiceover.txt").read_text(encoding="utf-8")
            captions = json.loads((video_dir / "captions.json").read_text(encoding="utf-8"))
        ...
        self.assertTrue(all(character.get("voice_profile", {}).get("sample_line") for character in bible["characters"]))
        self.assertEqual([scene["beat"] for scene in storyboard], ["Hook", "Conflict", "Escalation", "Twist", "Payoff"])
        self.assertGreaterEqual(sum(len(scene["dialogue"]) for scene in storyboard), 20)
        self.assertGreaterEqual(sum(scene["duration"] for scene in storyboard), 25)
        self.assertEqual(storyboard[-1]["time"], "25-30s")
        self.assertIn("dialogue", storyboard[0])
        self.assertIn("speaker_id", storyboard[0]["dialogue"][0])
        self.assertIn("NARRATOR:", voiceover)
        self.assertFalse(any(":" in str(item.get("word", "")) for item in captions))
        self.assertTrue(any(item.get("speaker") == "NARRATOR" for item in captions))
