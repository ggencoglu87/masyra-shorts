...
REAL_AI_VIDEO_PROVIDERS = {"veo3", "veo3fast", "runway", "kling", "pixverse", "hailuo", "replicate"}
IMAGE_MOTION_PROVIDERS = {"ltx", "scene_image_motion"}
QUALITY_THRESHOLDS = {
    "visual_quality": 75,
...
def generate_ai_videos_for_package(video_dir: Path, provider_name: str = "off", force: bool = False) -> dict:
    ...
    if not providers:
        result = {
            ...
            "real_ai_scene_count": 0,
            "image_motion_scene_count": 0,
            "image_only_scene_count": len(storyboard),
    ...
    image_only_scene_count = len([scene for scene in scenes if scene.get("scene_type") == "image_only" or not scene.get("file")])
...
def generate_ai_videos_for_dirs(video_dirs: list[Path], provider_name: str = "off", force: bool = False) -> dict:
    ...
    return {
        ...
        "real_ai_scene_count": sum(result.get("real_ai_scene_count", 0) for result in results),
        "image_motion_scene_count": sum(result.get("image_motion_scene_count", 0) for result in results),
        "image_only_scene_count": sum(result.get("image_only_scene_count", 0) for result in results),
...
def ai_scene_videos_available(video_dir: Path) -> bool:
    ...
    if result.get("real_ai_scene_count", 0) >= MIN_AI_SCENE_VIDEOS:
        return True
    scene_dir = video_dir / "scene-videos"
    real_ai_files = [
        scene
        for scene in result.get("scenes", [])
        if scene.get("scene_type") == "ai_video" and scene.get("file") and (video_dir / scene["file"]).exists()
    ]
    if real_ai_files:
        return len(real_ai_files) >= MIN_AI_SCENE_VIDEOS
    return False
...
def _scene_type_for_provider(provider_name: str) -> str:
    if provider_name in REAL_AI_VIDEO_PROVIDERS:
        return "ai_video"
    if provider_name in IMAGE_MOTION_PROVIDERS:
        return "image_motion"
    return "image_only"
